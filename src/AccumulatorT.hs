{-|
Module      : AccumulatorT
Description : Monad transformer for accumulating state
...
newtype AccumulatorT s i m a = AccumulatorT {
  runAccumulatorT :: (i -> m s) -- ^ function to turn the incomplete state into an accumulation
                  ...
                  -> m (a, [s], i) -- ^ result of accumulation
}
...
evalAccumulatorT :: (Monad m)
                 ...
                 -> m i -- ^ action to create a new incomplete state
                 -> m (a, [s], i) -- ^ result of accumulation
evalAccumulatorT (AccumulatorT f) fin fi = fi >>= f fin fi []
...
getAccumulation :: (Monad m) => AccumulatorT s i m [s]
...
accumulate v = AccumulatorT $ \_ _ st i -> return ((),st ++ [v], i)
